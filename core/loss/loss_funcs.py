...
class GaussianIIDLoss(nn.Module):
    def __init__(self, input_dim, proj_dim=64, n_modal=3, eps=1e-5):
        super(GaussianIIDLoss, self).__init__()
        self.eps = eps
    ...
    def compute_entropy(self, z):
        ...
        bs, dim = z.size()
        cov = torch.matmul(z.t(), z) / (bs - 1)

        cov_reg = cov + torch.eye(dim, device=z.device) * self.eps
        _, log_det = torch.linalg.slogdet(cov_reg)
    ...
    def compute_mi(self, x, y):
        ...
        h_xy = self.compute_entropy(torch.cat([x, y], dim=1))
        return h_x + h_y - h_xy

    def forward(self, wsi_emb, mol_emb, rpt_emb, mm_emb):
        parts_list = [wsi_emb, mol_emb, rpt_emb]

        proj_parts = [proj(p) for proj, p in zip(self.local_proj, parts_list)]
        proj_whole = self.global_proj(mm_emb)

        joint_parts = torch.cat(proj_parts, dim=1)
        mi_total = self.compute_mi(joint_parts, proj_whole)

        mi_sum_parts = 0
        for part in proj_parts:
            mi_sum_parts += self.compute_mi(part, proj_whole)

        phi = mi_total - mi_sum_parts
...
class InfoNCE(nn.Module):
    ...
    Args:
        ...
        reduction: Reduction method.md applied to the output.
