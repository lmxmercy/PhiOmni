...
class PathRNARptDatasetV2(Dataset):
    def __init__(
            self,
            wsi_emb_dir,
            mol_emb_dir,
        ...
        rpt_ids = [fname.split(".pt")[0] for fname in os.listdir(self.rpt_emb_dir) if fname.endswith(".pt")]

        self.case_ids = list(set(wsi_case_ids) & set(mol_ids) & set(rpt_ids))
        self.case_ids.sort()
    ...
    def _preload_embs_to_memory(self):
        for cid in self.case_ids:
            ...
            # load pathology report
            rpt_emb = torch.load(os.path.join(self.rpt_emb_dir, f"{cid}.pt"))["embedding"]
            rpt_emb = standardization(rpt_emb.unsqueeze(0)).squeeze(0) if self.if_rpt_std else rpt_emb
            self.cache_rpt[cid] = rpt_emb
