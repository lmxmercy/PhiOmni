...
os.environ['WANDB_API_KEY'] = 'your_wandb_api_key_here'  # set your wandb API key here if you want to log to wandb
import numpy as np
# --> Torch imports
import torch
...
# --> internal imports
from core.dataset.dataset_omni import PathRNARptDatasetV2, SlideDataset
...
from core.utils.learning import smooth_rank_measure, collate_path_rna_rpt, set_seed, collate_slide
...
def setup_input_dirs(args):
    ...
    report_dir = r'{}\embs\report\{}'.format(args["proj_dir"], args["type_of_report"])
    print("- Load pre-extracted {} report features from: {}".format(args["type_of_report"], report_dir))

    return pathology_dir, molecular_dir, report_dir


def setup_dataset(args):
    print("* Setup dataset...")
    pathology_dir, molecular_dir, report_dir = setup_input_dirs(args)
    dataset = PathRNARptDatasetV2(
        wsi_emb_dir=pathology_dir,
        mol_emb_dir=molecular_dir,
        rpt_emb_dir=report_dir,
...
def setup_ssl_model(args):
    ...
    ssl_model = PhiOmni(config=args).to(DEVICE)
...
def setup_loss_funcs(config):
    ...
    # loss_IID
    from core.loss.loss_funcs import GaussianIIDLoss
    loss_iid = GaussianIIDLoss(
...
def compute_losses(args, loss_funcs, wsi_emb, mol_emb, rpt_emb, mm_emb):
    losses = []
    ...
    losses.append((
        ...
        loss_fn_InterMod(query=wsi_emb, positive_key=rpt_emb, symmetric=args["symmetric_cl"])
    ) / 2 )

    # IID loss
    loss_iid = loss_funcs["iid"]
    losses.append(
        loss_iid(wsi_emb, mol_emb, rpt_emb, mm_emb) * args["gamma_iid"] 
    )
...
def train_loop(
    ...
    for b_idx, (patch_emb, mol_emb, rpt_emb) in enumerate(dataloader):
        s_fb = time.time()

        patch_emb = patch_emb.to(DEVICE)
        mol_emb = mol_emb.to(DEVICE)
        rpt_emb = rpt_emb.to(DEVICE)

        # forward pass
        wsi_emb, mol_emb, rpt_emb, mm_emb = ssl_model(patch_emb, mol_emb, rpt_emb)

        # compute losses
        loss = compute_losses(args, loss_funcs, wsi_emb, mol_emb, rpt_emb, mm_emb)
        ...
        ssl_model.eval()
        with torch.no_grad():
            wsi_emb_to_store, _, _, _ = ssl_model(patch_emb, None, None)
...
def inference_loop(ssl_model, val_dataloader):
    ...
    with torch.no_grad():
        for inputs, slide_id in tqdm(val_dataloader):
            ...
            wsi_embed = ssl_model.get_wsi_feature(inputs)
...
def main(args):
    ...
    EXP_CODE = "{}_lr{}_eps{}_bs{}_nToken{}_temp{}_conchv1_{}_{}_iid={}".format(
        args["model"],
        args["learning_rate"],
        args["epochs"],
        args["batch_size"],
    ...
    dataloader = DataLoader(
        ...
        shuffle=True,
        collate_fn=collate_path_rna_rpt,
    )
...
if __name__ == "__main__":
    ...
    args["type_of_report"] = "gatortron"
    args["n_tokens_mol"] = ENCODER_DIM_MAPPING[args["type_of_rna"]]
    args["n_tokens_rpt"] = ENCODER_DIM_MAPPING[args["type_of_report"]]
