...
from core.models.mils.layers import create_mlp
...
class TransMIL(MIL):
    def __init__(self, in_dim: int, embed_dim: int,
                 ...
                 num_attention_layers: int, n_classes: int, num_heads: int=8):
        """
        ...
        """
        super(TransMIL, self).__init__(in_dim=in_dim, embed_dim=embed_dim, n_classes=n_classes)
        ...
        self.classifier: nn.Linear = nn.Linear(embed_dim, n_classes)
    ...
    def forward_logits(
        ...
        wsi_feats, log_dict = self.forward_features(h, return_attention=False)
