...
from core.models.mils.layers import GlobalGatedAttention, GlobalAttention, create_mlp
...
class CLAMSB(MIL):
    ...
    def __init__(self,
                 ...
                 n_classes: int = 2,  # Number of output classes for bag-level prediction
        ...
        instance_classifiers = [nn.Linear(embed_dim, 2) for _ in range(n_classes)]  # Binary classifier per class
    ...
    def forward_head(self, h: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            logits (torch.Tensor): Raw classification logits [B, n_classes].
        """
    ...
    def forward_instance_heads(self, h: torch.Tensor, attention_scores: torch.Tensor,
        ...
        inst_labels = F.one_hot(label, num_classes=self.n_classes).squeeze(0).to(label.device)
    ...
    def forward_logits(
        ...
        slide_feats, intermeds = self.forward_features(h, return_attention=False)
